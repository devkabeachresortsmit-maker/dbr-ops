    ...
    db.collection("users").doc(user.uid).get()
      .then(doc => {
        if (doc.exists) {
          ...
          if (userInitialEl) userInitialEl.textContent = initialChar;
          if (mobileUserInitialEl) mobileUserInitialEl.textContent = initialChar;
...
function renderUsersTable(usersList) {
  ...
  tbody.innerHTML = usersList.map(u => {
    ...
    // Check if it's the admin user itself to prevent deleting self
    ...
    return `
      <tr>
        ...
        <td>
          ${isSelf ? '<span style="color:var(--text-muted); font-size:12px;">Admin (Active)</span>' : `
            <button class="btn btn-glass delete-user-btn" data-uid="${u.uid || u.username}" data-username="${u.username}" style="padding: 4px 8px; font-size:11px; border-color:var(--status-rejected); color:var(--status-rejected); background: transparent;">
              <i data-lucide="trash-2" style="width:12px;height:12px;display:inline-block;vertical-align:middle;margin-right:4px;"></i>Delete
            </button>
          `}
        </td>
      </tr>
    `;
  }).join("") || `<tr><td colspan="5" style="text-align:center;color:var(--text-muted);">No users registered.</td></tr>`;

  // Bind click handlers to the delete buttons
  tbody.querySelectorAll(".delete-user-btn").forEach(btn => {
    btn.addEventListener("click", () => {
      const uid = btn.dataset.uid;
      const username = btn.dataset.username;
      confirmAndDeleteUser(uid, username);
    });
  });
...
function ensureDeletePasswordExists() {
  db.collection("settings").doc("security").get()
    ...
    .catch(err => {
      console.warn("Could not check/create settings/security in firestore:", err);
    });
}

function confirmAndDeleteUser(uid, username) {
  const passcode = prompt(`To delete user "${username}", please enter the authorization password:`);
  if (passcode === null) return; // User cancelled

  showToast("Verifying authorization passcode...");
  ...
  db.collection("settings").doc("security").get()
    .then(doc => {
      ...
      return doc.data().deletePasscode || "6363";
    })
    .catch(err => {
      console.warn("Failed to read settings/security from firestore, falling back to local verification:", err);
      return "6363";
    })
    .then(correctPasscode => {
      if (passcode !== correctPasscode) {
        showToast("Incorrect delete authorization password.");
        return;
      }

      showToast(`Deleting user "${username}"...`);

      // Delete from firestore "users" collection
      return db.collection("users").doc(uid).delete()
        .then(() => {
          showToast(`User "${username}" profile successfully deleted.`);
          logActivity(`Super Admin deleted user "${username}".`);
          addNotification(`User ${username} deleted by Admin.`);
        });
    })
    .catch(err => {
